    ...
    def __init__(self, model, modelCS, modelFS, output_path: str, opt):
        ...
        if opt.fixed_code:
            self.start_code = torch.randn(
                [opt.n_samples, opt.C, opt.H // opt.f, opt.W // opt.f], device="cuda"
            )
                ...
                with self.precision_scope("cuda"):
                    ...
                    if len(subprompts) > 1:
                        ...
                        for i in range(len(subprompts)):
                            ...
                            c = torch.add(
                                c,
                                self.modelCS.get_learned_conditioning(subprompts[i]),
                                alpha=weight,
                            )
                    else:
            ...
            for prompts in tqdm(data, desc="data"):
                ...
                with self.precision_scope("cuda"):
                    ...
                    for batch_index in range(batch_size):
                        x_samples_ddim = self.modelFS.decode_first_stage(
                            samples_ddim[batch_index].unsqueeze(0)
                        )
                        x_sample = torch.clamp(
                            (x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0
                        )
